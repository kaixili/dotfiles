...
run_once("google-chrome-stable --no-startup-window &")
run_once("fcitx &")
-- }}}
...
-- {{{ Tags
tags = {
  names = { '[1:Main]', '[2:Chrome]', '[3:Term]', '[4]' ,'[5:Music]', '[6:File]', '[7:IM]', '[8:Other]' },
  layout = { layouts[2], layouts[5], layouts[2], layouts[1], layouts[1], layouts[4], layouts[1], layouts[1] }
}
for s = 1, screen.count() do
...
if beautiful.wallpaper then
  for s = 1, screen.count() do
    gears.wallpaper.maximized(os.getenv("HOME") .. "/Pictures/wallpaper.jpg", s, true)
  end
  for s = 2, screen.count() do
    gears.wallpaper.maximized(os.getenv("HOME") .. "/Pictures/wallpaper2.jpg", s, true)
  end
...
--myweather = lain.widgets.weather({
  ...
  memwidget = lain.widgets.mem({
    settings = function()
      widget:set_markup(markup("#e0da37", mem_now.percent .. "% "))
    end
  })
...
awful.key({ altkey }, "p", function() os.execute("scrot -s ~/tmp/`date +'%Y-%m-%dT%H:%M:%S'`.png") end),
...
-- Dropdown terminal
awful.key({ modkey,	        }, "z",      function () quakeconsole[mouse.screen.index]:toggle() end),
...
awful.key({ modkey,           }, "l",
function ()
  os.execute("i3lock-fancy -p")
end),

-- Brightness control
awful.key({ }, "XF86MonBrightnessDown",
function ()
  os.execute("xbacklight -dec 4")
end),
awful.key({ }, "XF86MonBrightnessUp",
function ()
  os.execute("xbacklight -inc 4")
end),
...
--  os.execute(string.format("amixer set Headphone toggle", volumewidget.channel))
--  volumewidget.update()
--end),
...
awful.key({ altkey,           }, "space",  function (c) c.ontop = not c.ontop            end),
awful.key({ modkey,           }, "n",
function (c)
...
function(c)
  if c.maximized_horizontal == true and c.maximized_vertical == true or c.maximized == true then
    c.border_width = 0
